...
function GmailIcon({ className = "h-5 w-5" }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none">
      ...
      <path d="M24 5.457v13.909c0 .904-.732 1.636-1.636 1.636h-3.819V11.73L12 16.64l-6.545-4.91v9.273H1.636A1.636 1.636 0 0 1 0 19.366V5.457c0-2.023 2.309-3.178 3.927-1.964L5.455 4.64 12 9.548l6.545-4.91 1.528-1.145C21.69 2.28 24 3.434 24 5.457z" fill="url(#gmail_grad)" />
      <defs>
...
function OutlookIcon({ className = "h-5 w-5" }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none">
      <path d="M24 7.387v10.478c0 .23-.08.424-.238.583a.793.793 0 0 1-.584.238h-8.322V6.567h8.322c.23 0 .425.08.584.238.159.159.238.352.238.582z" fill="#0364B8" />
      <path d="M16.58 6.567v12.119l-1.784.91L8.322 22.5V1.5l6.474 2.904 1.784.91v1.253z" fill="#0A2767" />
      <path d="M16.58 6.567H8.322v12.119l8.258-3.373V6.567z" fill="#28A8EA" />
      <path d="M10.903 9.322c-.314-.342-.728-.513-1.242-.513-.56 0-1.003.19-1.328.571-.326.38-.489.876-.489 1.487 0 .626.161 1.13.484 1.51.323.38.755.571 1.296.571.527 0 .95-.172 1.27-.516.32-.344.48-.823.48-1.437 0-.638-.157-1.14-.471-1.503v-.17zm.89-.667c.482.506.723 1.182.723 2.03 0 .838-.246 1.52-.738 2.047-.492.526-1.13.79-1.914.79-.77 0-1.39-.26-1.864-.78-.473-.521-.71-1.196-.71-2.027 0-.862.242-1.554.726-2.075.484-.521 1.113-.782 1.886-.782.786 0 1.409.266 1.891.797z" fill="white" />
    </svg>
  );
}
...
function YahooIcon({ className = "h-5 w-5" }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none">
      ...
      <path d="M7.2 7l3.3 6.9h.1L13.8 7H16l-4.5 9.5V20h-2.1v-3.5L5 7h2.2z" fill="white" />
...
function ZohoIcon({ className = "h-5 w-5" }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none">
      ...
      <path d="M5 8h5.5l-5.5 8h5.5" stroke="white" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" fill="none" />
      <path d="M13.5 8h5.5l-5.5 8h5.5" stroke="white" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" fill="none" />
    </svg>
  );
}
...
function ProtonIcon({ className = "h-5 w-5" }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none">
      ...
      <path d="M6 10.5L12 7l6 3.5v4L12 18l-6-3.5v-4z" fill="white" fillOpacity=".2" stroke="white" strokeWidth="1.2" />
...
function DefaultMailIcon({ className = "h-5 w-5" }: { className?: string }) {
  return (
    <div className={`${className} rounded bg-gradient-to-br from-blue-500 to-cyan-500 flex items-center justify-center`}>
...
interface EmailProvider {
  ...
  buildWebUrl: (to: string, subject: string, body: string, cc: string, bcc: string) => string;
  isDefault?: boolean;
}
...
function buildMailto(to: string, subject: string, body: string, cc: string, bcc: string) {
...
const EMAIL_PROVIDERS: EmailProvider[] = [
  ...
  },
  {
    ...
    icon: GmailIcon,
    appSchemes: {
      android: "intent://compose?#Intent;scheme=mailto;package=com.google.android.gm;end",
      ios: "googlegmail://co",
    },
  ...
  },
  {
    ...
    icon: OutlookIcon,
    appSchemes: {
      android: "intent://compose?#Intent;scheme=mailto;package=com.microsoft.office.outlook;end",
  ...
  },
  {
    ...
    icon: YahooIcon,
    appSchemes: {
      android: "intent://compose?#Intent;scheme=mailto;package=com.yahoo.mobile.client.android.mail;end",
  ...
  },
  {
    ...
    icon: ZohoIcon,
    appSchemes: {
      android: "intent://compose?#Intent;scheme=mailto;package=com.zoho.mail;end",
  ...
  },
  {
    ...
    icon: ProtonIcon,
    appSchemes: {
      android: "intent://compose?#Intent;scheme=mailto;package=ch.protonmail.android;end",
...
export function EmailDocumentModal({
  ...
  const selectedProvider =
    EMAIL_PROVIDERS.find((p) => p.id === selectedProviderId) ?? EMAIL_PROVIDERS[0];
  ...
  const handleSendEmail = useCallback(() => {
    ...
    if (os !== "desktop") {
      const scheme = os === "ios" ? selectedProvider.appSchemes.ios : selectedProvider.appSchemes.android;
      if (scheme) {
        if (os === "ios") {
          const params = `?to=${enc(to)}&subject=${enc(sub)}&body=${enc(plainBody)}${cc ? `&cc=${enc(cc)}` : ""}${bcc ? `&bcc=${enc(bcc)}` : ""}`;
          const appUrl = scheme + params;
          const webUrl = selectedProvider.buildWebUrl(to, sub, plainBody, cc, bcc);
          ...
          const fallbackUrl = selectedProvider.buildWebUrl(to, sub, plainBody, cc, bcc);
          const intentUrl = intentBase.replace(";end", `;S.browser_fallback_url=${enc(fallbackUrl)};end`);
    ...
    window.open(selectedProvider.buildWebUrl(to, sub, plainBody, cc, bcc), "_blank", "noopener,noreferrer");
  }, [recipientEmail, ccEmail, bccEmail, subject, defaultSubject, plainBody, selectedProvider, os]);
